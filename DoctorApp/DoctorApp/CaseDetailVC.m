...
//    NSLog(@"%@",self.myCase);
    ...
    for(int i = 0; i < [requirementArray count]; i++)
    {
        ...
        NSString *author = [NSString stringWithFormat:@"%@ %@",d[@"_creator"][@"firstname"][@"en_us"],d[@"_creator"][@"lastname"][@"en_us"]];
    ...
    id s = self.myCase[@"case"][@"patient_info"][@"smoking"];
    if([s boolValue])
        smokeLabel.text = @"Yes";
    else
        smokeLabel.text = @"No";
    ...
    id ss = self.myCase[@"case"][@"patient_info"][@"drinking"];
    if([ss boolValue])
        drinkLabel.text = @"Yes";
    else
        drinkLabel.text = @"No";
    ...
    UILabel *issueTxt = [[UILabel alloc] initWithFrame:CGRectMake(20, 50, 240, 50)];
    ...
    for(int i = 0; i < [issueArray count]; i++)
    {
        ...
        if(!answer)
            answer = @"";
        [issueString appendString:[NSString stringWithFormat:@"<p>Q:    %@</p><p></p><p>A:    %@</p><p></p><p></p>",question,answer]];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"actionVC"]) {
        ActionsVC *targetVC = (ActionsVC*)segue.destinationViewController;
        NSString *caseidString = self.myCase[@"case"][@"_id"];
        targetVC.caseid = caseidString;
    }
}
- (IBAction)action:(id)sender {
    [self performSegueWithIdentifier:@"actionVC" sender:self];
}
